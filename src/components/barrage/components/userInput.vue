...
export default {
  ...
  methods : {
    ...
    // 设置弹幕输入框内容
    setBarraageContent () {
      let sessionBarrageContent = sessionStorage.getItem(sessionStorageField.BARRAGECONTENT)
    ...
    changeBodyScrollTop (value) {
      document.documentElement ? (document.documentElement.scrollTop = value) : ( document.body.scrollTop  = value)
    },
    // 处理ios 输入框获取焦点 页面滑动问题
    handeliosKeyBoardPrevDetault() {
      document.body.addEventListener('focusin' ,() => this.handelFocusinEvent() ),
      document.body.addEventListener ('focusout' , () => this.handelFocusoutEvent())
    },
    //  处理 ios input 输入框获取焦点被软键盘 覆盖问题
    handleIosInputKeyBoardCover () {
      let ua = navigator.userAgent
      let inputDistanceTop = this.$refs.barrageinput.getBoundingClientRect().top
